...
public class HigherOrderFunctionsLearningTest {
    ...
    public void testDistinct() {
        Stream<String> input = Stream.of("a1", "a2", "a3", "b1", "b2", "b3");
        List<String> actual = input.map(x -> x.substring(0,1)).distinct().collect(Collectors.toList());
    ...
    public void testConcatEven() {
        Stream<String> input = Stream.of("a1", "a2", "a3", "b1", "b2", "b3");
        String actual = input.filter(x -> x.endsWith("2")).collect(Collectors.joining());
    ...
    public void testCountNonBots() {
        Stream<Revision> input = getRevisions("soup30.json");
        long actual = input.filter(x -> !x.user.toLowerCase().contains("bot")).count();
        int expected = 26;
        Assertions.assertEquals(expected, actual);
    }
    ...
    public void testCountChangesInFebruary() {
        Stream<Revision> input = getRevisions("soup30.json");
        long actual = input.filter(x -> x.timestamp.atZone(ZoneId.of("UTC")).getMonth().equals(Month.FEBRUARY)).count();
        int expected = 9;
        Assertions.assertEquals(expected, actual);
    }
    ...
    public void testCountChangesByMonth() {
        Stream<Revision> input = getRevisions("soup04.json");
        Map<Month, Long> actual = input.collect(Collectors.groupingBy(x -> x.timestamp.atZone(ZoneId.of("UTC")).getMonth(),Collectors.counting()));
    ...
    public void testDetermineMostActiveMonth() {
        Stream<Revision> input = getRevisions("soup30.json");
        Month actual = input.collect(Collectors.groupingBy(x -> x.timestamp.atZone(ZoneId.of("UTC")).getMonth(),Collectors.counting()))
